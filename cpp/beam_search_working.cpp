...
struct SrcView {
    py::object src_obj;                 // Original SubgraphMetadata
    std::u32string ref;
    std::u32string hyp;
    int ref_max_idx;
    int hyp_max_idx;
    ...
    py::object backtrace_node_set;      // Python set of (hyp, ref)
    py::object unambiguous_matches;     // Python set of (hyp, ref)
};
...
static SrcView make_src_view(const py::object& src) {
    ...
    v.backtrace_node_set = src.attr("backtrace_node_set");
    v.unambiguous_matches = src.attr("unambiguous_matches");
...
static inline bool end_segment(Path& p) {
    ...
    if (hyp_is_empty) {
        ...
        p._at_unambiguous_match_node =
            is_match_segment && in_set_of_pairs(p.src->unambiguous_matches, p.hyp_idx, p.ref_idx);
        p._end_indices.emplace_back(p.hyp_idx, p.ref_idx, p._open_cost);
    }
...
static int add_substitution_or_match(const Path& parent, Path& out_child) {
    ...
    if (!is_match) {
        bool is_backtrace = in_set_of_pairs(parent.src->backtrace_node_set, parent.hyp_idx, parent.ref_idx);
...
static py::object error_align_beam_search_cpp(py::object src_obj, int beam_size) {
    ...
    while (!beam.empty()) {
        ...
        // sort by norm_cost and keep top beam_size
