...
→ **Update [2026-06-22]:** As of version `0.1.0b10`, the word-level pass defaults to a faster `rapidfuzz`-based method that anchors matches from a single optimal Levenshtein alignment. The original graph-based pass is still available via `error_align(ref, hyp, word_level_method="unambiguous")`.
