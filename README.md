...
→ **Update [2026-06-22]:** As of version `0.1.0b10`, the word-level pass defaults to a faster `rapidfuzz`-based method that anchors matches from a single optimal Levenshtein alignment. Computing the match anchors this way is far cheaper than building the full backtrace graph, and it also commits to more anchors, leaving only smaller spans for beam search. Both effects compound on longer examples, where the speedup is largest. The original graph-based pass is still available via `error_align(ref, hyp, word_level_method="unambiguous")`.
